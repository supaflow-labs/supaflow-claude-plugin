...
Destructive tools: `datasources_delete`, `pipelines_delete`, `schedules_delete`, `agent_remove` (always -- it force-removes the container; with `purge: true` it also deletes the agent identity volume, so the next start enrolls a brand-new agent), and `pipelines_sync` when `reset_target: true`.
