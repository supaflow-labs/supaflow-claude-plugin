...
- Before `mcp__supaflow__jobs_cancel`: require the exact tool to be present, call `jobs_status`, continue only for a `queued`, `picked`, or `running` job, show its exact id/status/message and cancellation impact, then get explicit confirmation. MCP approval alone is not confirmation. After cancellation, call `jobs_status` again and report the verified database state.
